...
func TestCreateWithExistingTimestamp(t *testing.T) {
	...
	curTime, err := Parse("2016-01-01 00:00:00")
	if err != nil {
		panic(err)
	}
	...
	tests.AssertEqual(t, user.CreatedAt, curTime)
	tests.AssertEqual(t, user.UpdatedAt, curTime)

	var newUser User
	DB.First(&newUser, user.ID)

	tests.AssertEqual(t, newUser.CreatedAt, curTime)
	tests.AssertEqual(t, newUser.UpdatedAt, curTime)
}
...
func TestCreateFromSubQuery(t *testing.T) {
	...
	result := DB.Session(&gorm.Session{DryRun: true}).Model(&Pet{}).Create([]map[string]interface{}{
		{
			"name":    "cat",
			"user_id": gorm.Expr("(?)", DB.Table("(?) tmp", subQuery).Select("@uid:=id")),
		},
		{
			"name":    "dog",
	...
	if !regexp.MustCompile(`INSERT INTO .pets. \(.name.,.user_id.\) .*VALUES \(.+,\(SELECT @uid:=id FROM \(SELECT id FROM .users. WHERE name=.+\) tmp\)\),\(.+,@uid\)`).MatchString(result.Statement.SQL.String()) {
...
func TestCreateWithAutoIncrementCompositeKey(t *testing.T) {
	...
	if err := DB.First(&newProd).Error; err != nil {
		...
		tests.AssertObjEqual(t, newProd, prod, "ProductID", "LanguageCode", "Code", "Name")
	}
}
...
func TestCreateOnConflictWithDefaultNull(t *testing.T) {
	...
	if err := DB.Migrator().DropTable(&OnConflictUser{}); err != nil {
		if !strings.Contains(err.Error(), "ORA-00942") {
	...
	tests.AssertEqual(t, err, nil)
	tests.AssertEqual(t, u2.Name, "on-conflict-user-name-2")
	tests.AssertEqual(t, u2.Email, "on-conflict-user-email-2")
	tests.AssertEqual(t, u2.Mobile, "133xxxx")
}

func TestCreateFromMapWithoutPK(t *testing.T) {
	t.Skipf("TBD: Need to decide whether the Oracle driver supports this")
...
func TestCreateFromMapWithTable(t *testing.T) {
	...
	supportLastInsertID := DB.Dialector.Name() == "mysql" || DB.Dialector.Name() == "sqlite"
